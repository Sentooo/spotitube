...
import cassete from "../../assets/img/SpotitubeLogo.png";
...
function ArtistUpload() {
  ...
  const onDrop = async (acceptedFiles) => {
    const newAudioInfo = await Promise.all(
  ...
  return (
    <ArtistLayout active={"Upload"}>
      <div
        className={`col-10 h-150 d-flex align-align-items-center justify-content-center p-5 text-light overflow-auto ${styles.contentContainer}`}
      >
        <ToastContainer />
        <form
          className={`w-75 d-flex flex-column gap-3 ${styles["upload-form"]}`}
        ...
        >
          ...
          <div className={`card`}>
            <div className="card-header">Edit & Review</div>
            <div
              className={`${styles.tracksContainer} card-body d-flex flex-column gap-3`}
            >
              {audioFiles.length !== 0 ? (
                audioFiles.map((file, index) => (
                  <div key={index} className={`${styles.trackCard}`}>
                    ...
                    <Select
                      ...
                      isMulti
                      name="genre"
                      defaultValue={audioInfo[index].genres.map((genre) => ({
                        value: genre,
                        label: genre,
                      }))}
                      options={musicGenres}
            ...
            <Card.Body>
              <div className={`${styles.albumContainer}`}>
                ...
                <div className={`${styles.albumImageContainer}`}>
                  ...
                  />
                  <img
                    src={albumDetails.cover != null ? image : uploadImage}
                    alt=""
