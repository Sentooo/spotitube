...
function HeaderMP({ verified }) {
  ...
  return (
    <header className="w-100 p-2 row m-0 text-light bg-black">
      <RequestForm show={show} handleClose={() => setShow(false)} />
      <div className={`col-lg-2 col-md-2 col-sm-4 d-flex gap-2 `}>
        <div
          className={`col-3 d-flex align-items-center justify-content-start`}
        >
      ...
      <div className="col-lg-3 col-md-4 col-sm-2 d-flex justify-content-end align-items-center">
        {role != "listener" ? (
          ...
          <button
            className={`btn applyArtistBtn ${
              requestStatus
                ? "btn-success cursor-notAllowed"
                : "applyArtistBtn-red"
            }`}
            onClick={(e) => handleOpen(e)}
            disabled={!verified}
          >
            {requestStatus ? "Request Pending" : "Become an Artist"}
          </button>
        )}
        <Menu
          anchorEl={anchorEl}
        ...
        >
          <Link to={"/studio/content"}>
            <MenuItem
              ...
              sx={{ "&:hover": { backgroundColor: "#c20000" }, color: "white" }}
            >
              Music
            </MenuItem>
          </Link>
          <MenuItem
            onClick={handleMenuClose}
            sx={{ "&:hover": { backgroundColor: "#c20000" } }}
          >
            Podcast
          </MenuItem>
          <MenuItem
